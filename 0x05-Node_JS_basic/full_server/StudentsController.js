...
class StudentsController {
  static getAllStudents(_, response) {
    ...
    readDatabase(dataPath)
      .then((studentGroups) => {
        const respData = ['This is the list of our students'];
        ...
        for (const [field, group] of Object.entries(studentGroups).sort(sorter)) {
          respData.push([
        ...
        response.status(200).send(respData.join('\n'));
      })
      .catch((err) => {
        response
          .status(500)
    ...
    readDatabase(dataPath)
      .then((studentGroups) => {
        ...
        if (Object.keys(studentGroups).includes(major)) {
          ...
          responseText = `List: ${group.map((st) => st.firstname).join(', ')}`;
        }
